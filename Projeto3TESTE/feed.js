...
function Feed({ projetos, projetosEmAlta, aoClicarNoProjeto }) {
    ...
    const [tempoMaxDias, setTempoMaxDias] = React.useState(0); // 0 = exibindo todos
    const ORCAMENTO_TETO = 10000;
    const [orcamentoMax, setOrcamentoMax] = React.useState(0); // 0 = exibindo todos

    const filtros = ['Todos', 'Fácil', 'Médio', 'Difícil'];

    function normalizarDif(d) {
        return d; // já não precisa normalizar, comparamos direto
    }
